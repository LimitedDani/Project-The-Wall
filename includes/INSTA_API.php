...
class vacature {
    ...
    static function loadVacaturesSettings($mysqli, $parkid)
    {
        ...
        if ($count > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<td><button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#' . $row["ID"] . '">Edit</button><a href="parksettings.php?id=' . $parkid . '&removevacature=' . $row["ID"] . '&jobs" class="btn btn-danger btn-sm">Verwijderen</a></td>';
    ...
    static function addVacature($mysqli, $parkid, $name, $body)
    {
        setlocale(LC_TIME, 'NL_nl');
        $name = strip_tags($name);
        $sql = "INSERT INTO pco_parks_jobs (park_id, job_name, job_description, job_status) VALUES ('$parkid', '$name', '', '0');";
        $result = mysqli_query($mysqli, $sql);
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '
                        <div class="jumbotron hover" id="' . $id . '">
                            ...
                            <span>'.vacature::countApplications($mysqli, $id).' sollicitant(en)</span></span>
                        </div>
